    ...
    public class BuildScriptPackedMode : BuildScriptBase
    {
            ...
            AddressableAssetsBuildContext aaContext)
        {
            ...
            if (!m_CreatedProviderIds.Contains(bundledProviderId))
            {
                ...
                var bundleProviderData = ObjectInitializationData.CreateSerializedInitializationData(bundleProviderType, bundledProviderId, schema.DataStreamProcessorType);
        ...
        static void MoveFileToDestinationWithTimestampIfDifferent(BundledAssetGroupSchema schema, string srcPath, string destPath, ContentCatalogDataEntry dataEntry, IBuildLogger log)
        {
            if (srcPath == destPath)
                return;
            ...
            using (log.ScopedStep(LogLevel.Verbose, "Move File", $"{srcPath} -> {destPath}"))
            {
                ...
                if (schema.DataStreamProcessorType.Value != null) {
                    using (var src = new FileStream(srcPath, FileMode.Open, FileAccess.Read, FileShare.Read, 1024 * 1204, FileOptions.Asynchronous))
                    {
                        ...
                        {
                            var dsp = Activator.CreateInstance(schema.DataStreamProcessorType.Value) as IDataConverter;
                            using (var writeStr = dsp.CreateWriteStream(dst, dataEntry.InternalId))
                                src.CopyTo(writeStr);
                        }
                    }
                } else {
        ...
        void PostProcessBundles(AddressableAssetGroup assetGroup, List<string> buildBundles, List<string> outputBundles, IBundleBuildResults buildResult, ResourceManagerRuntimeData runtimeData, List<ContentCatalogDataEntry> locations, FileRegistry registry, Dictionary<string, ContentCatalogDataEntry> primaryKeyToCatalogEntry, Dictionary<string, string> bundleRenameMap, List<Action> postCatalogUpdateCallbacks)
        {
            ...
            for (int i = 0; i < buildBundles.Count; ++i)
            {
                ...
                MoveFileToDestinationWithTimestampIfDifferent(schema, srcPath, targetPath, dataEntry, m_Log);
